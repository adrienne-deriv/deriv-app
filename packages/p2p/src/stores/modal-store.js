...
export default class ModalStore extends BaseStore {
    ...
    @action.bound
    onMount() {
        console.log('DAFUQ IM CALLED AGAIN');
        // only this reaction can modify is_modal_open and modal_id, NO ONE ELSE CAN DO IT!
        const disposer = reaction(
            () => this.current_modal,
            () => {
                console.log('wanna show a modal?', this.current_modal);
                ...
                // case when hideModal() is called
                // this.setIsModalOpen(!this.is_modal_open);
                // if (this.should_switch_modal) {
                //     throttle(() => {
                //         this.setModalId(this.current_modal);
                //         this.setIsModalOpen(true);
                //     }, this.MODAL_TRANSITION_DELAY);
                //     this.should_switch_modal = false;
                // } else {
                //     // we let the single modal close first, then only let ModalManager unmount it by switching the modal_id
                //     throttle(() => this.setModalId(this.current_modal), this.MODAL_TRANSITION_DELAY);
                // }
    ...
    passModalProps(modal_id, modal_props) {
        console.log('[passModalProps] passed modal props', modal_props);
